# -*- coding: utf-8 -*-
"""
Serializers para certificados digitales - CORREGIDO
"""
...
class CertificateUploadSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        # Crear certificado con todos los campos requeridos
        certificate = DigitalCertificate(
            company=company,
            certificate_file=certificate_file,
            environment=environment,
            status='ACTIVE',
            # Campos requeridos con valores por defecto seguros
            subject_name=f'Certificado {company.business_name}',
            issuer_name='Autoridad Certificadora SRI',
            serial_number=str(uuid.uuid4())[:20],
            valid_from=now,
            valid_to=now + timezone.timedelta(days=365),
            fingerprint=str(uuid.uuid4()).replace('-', '')[:32],  # Temporal
            password_hash='temp'  # Temporal
        )
        
        # Configurar contraseña real
        certificate.set_password(password)
        
        # Guardar - el modelo se encargará de generar el fingerprint real
        certificate.save()
